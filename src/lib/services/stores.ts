 ...
 */
export const addRecordToStore = <T>(key: string, newRecord: T, store: Writable<Record<string, T>>) => store.update(records => ({
    [key]: newRecord,
    ...records
  }));
   ...
   * @param store the store it will be updating 
   */
  export const removeRecordFromStore = <T>(key: string, store: Writable<Record<string, T>>) => store.update(records => {
   ...
   * @param store the store it will be updating 
   */
  export const updateRecordInStore = <T>(key: string, newRecord: T, store: Writable<Record<string, T>>) => store.update(records => {
